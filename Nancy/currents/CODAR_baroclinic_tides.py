...
def save_netcdf_single_depth(tide, depth, const, lons, lats, to, tf):
    fname = '{}_CODAR_surface_tides_{}_{}.nc'.format(const,
                                                     to.strftime('%Y%m%d'),
                                                     tf.strftime('%Y%m%d'))
    nc_file = nc.Dataset(fname, 'w', zlib=True)
    # dataset attributes
    nc_tools.init_dataset_attrs(
        nc_file,
        title='{} tides'.format(const),
        notebook_name='N/A',
        nc_filepath='/data/nsoontie/MEOPAR/analysis/Nancy/tides/' + fname,
        comment='Tidal analysis depth {}'.format(depth))
    # dimensions
    temp = tide['Phase']
    nc_file.createDimension('y', temp.shape[0])
    nc_file.createDimension('x', temp.shape[1])
    # variables
    # lat, lon
    lon = nc_file.createVariable('nav_lon', 'float32', ('y', 'x'), zlib=True)
    lon[:] = lons[:]
...
lons = f.variables['nav_lon'][1:, 1:]
lats = f.variables['nav_lat'][1:, 1:]
nconst = 2
...
for const in baroclinic_ellipse:
    save_netcdf_depthprofile(baroclinic_ellipse[const], depths, const,
                             lons, lats, to, tf)
